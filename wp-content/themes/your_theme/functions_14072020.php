...
function remove_jq_migrate( $scripts ) { // Отключение jquery-migrate
...
if (!function_exists('add_scripts')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_scripts() { // добавление скриптов
	    ...
	    // wp_enqueue_script('jquery', get_template_directory_uri().'/js/jquery-3.4.1.min.js','','',true); // добавляем свой
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
	    if(is_admin()) return false; // если мы в админке - ничего не делаем
	    wp_enqueue_style( 'bs', get_template_directory_uri().'/css/bootstrap-grid.min.css' ); // бутстрап
...
// 		wp_enqueue_style( 'main', get_template_directory_uri().'/css/main.css' ); // основные стили шаблона
		wp_enqueue_style( 'font-awesome1', get_template_directory_uri().'/css/all.min.css' ); // иконки fontawesome
		wp_enqueue_style( 'main', get_template_directory_uri().'/css/main.css' ); // основные стили шаблона
	}
}
...
// add_filter( 'rwmb_meta_boxes', 'uraltur_register_meta_boxes' );
// function uraltur_register_meta_boxes( $meta_boxes )
// {
// 	// meta box Цена
// 	$meta_boxes[] = array(
// 			'id'       => 'offer',
